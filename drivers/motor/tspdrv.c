...
static void regulator_onoff(int on)
{
	int ret = 0;
	static struct regulator *vib_power;

	pr_info("[VIB] %s, on=%d\n", __func__, on);

	if (!vib_power) {
		vib_power = regulator_get(NULL, vibrator_drvdata.power_name);
		if (IS_ERR(vib_power)) {
			printk(KERN_ERR"could not get %s, rc = %ld\n",
					vibrator_drvdata.power_name, PTR_ERR(vib_power));
			vib_power = NULL;
			return;
		}
	}
	...
	if (on) {
		ret = regulator_set_voltage(vib_power, vibrator_drvdata.power_volt, vibrator_drvdata.power_volt);
		if (ret) {
			printk(KERN_ERR"failed set voltage %s, rc=%d\n",
					vibrator_drvdata.power_name, ret);
			return;
		}
		ret = regulator_enable(vib_power);
		if (ret) {
			printk(KERN_ERR"enable %s failed, rc=%d\n",
					vibrator_drvdata.power_name, ret);
			return;
		}
		printk(KERN_DEBUG"haptic power_on is finished.\n");
	} else {
		if (regulator_is_enabled(vib_power)) {
			ret = regulator_disable(vib_power);
			if (ret) {
				printk(KERN_ERR"disable %s failed, rc=%d\n",
						vibrator_drvdata.power_name, ret);
				return;
			}
		}
...
int vibetonz_clk_on(struct device *dev)
{
	...
	clk_prepare_enable(vibetonz_clk);
...
int vibetonz_clk_off(struct device *dev)
{
	...
	clk_disable_unprepare(vibetonz_clk);
	clk_put(vibetonz_clk);
...
static int tspdrv_parse_dt(struct platform_device *pdev)
{
	...
	vibrator_drvdata.vib_en_gpio = of_get_named_gpio(np, "samsung,vib_en", 0);
	if (!gpio_is_valid(vibrator_drvdata.vib_en_gpio)) {
		vibrator_drvdata.flag_en_gpio = 0;
		pr_info("%s:%d, en gpio not specified\n", __func__, __LINE__);
	} else {
		vibrator_drvdata.flag_en_gpio = 1;
	}

	vibrator_drvdata.vib_pwm_gpio = of_get_named_gpio(np, "samsung,vib_pwm", 0);
	if (!gpio_is_valid(vibrator_drvdata.vib_pwm_gpio)) {
		pr_err("%s:%d, reset gpio not specified\n",
				__func__, __LINE__);
	}

	rc = of_property_read_string(np, "samsung,ldo_name", &vibrator_drvdata.power_name);
	if (rc < 0) {
		pr_err("[%s]: Unable to read ldo_name\n", __func__);
		vibrator_drvdata.power_name = NULL;
		vibrator_drvdata.power_volt = 0;
	} else {
		rc = of_property_read_u32(np, "samsung,ldo_volt", &vibrator_drvdata.power_volt);
		if (rc < 0) {
			pr_err("[%s] Unable to read ldo_volt\n", __func__);
		}

		pr_info("[%s] ldo_name[%s] ldo_volt[%d]\n", __func__,
				vibrator_drvdata.power_name, vibrator_drvdata.power_volt);
	}
	...
	if (rc) {
		pr_info("gp_clk not specified so using GP2\n");
...
static int tspdrv_probe(struct platform_device *pdev)
{
	...
	if (vibrator_drvdata.power_name != NULL)
		vibrator_drvdata.power_onoff = regulator_onoff;
	vibrator_drvdata.pwm_dev = NULL;
	vibrator_drvdata.dev = &pdev->dev;
