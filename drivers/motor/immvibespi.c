	...
	if (g_bampenabled) {
		...
		if (vibrator_drvdata.vib_model == HAPTIC_PWM) {
			if(vibrator_drvdata.is_pmic_vib_pwm){  //PMIC PWM
				gpio_set_value(vibrator_drvdata.vib_pwm_gpio, VIBRATION_OFF);
				if(vibrator_drvdata.pwm_dev != NULL) //Disable the PWM device.
					pwm_disable(vibrator_drvdata.pwm_dev);
			} else{	//AP PWM
				motor_pinctrl = devm_pinctrl_get_select(vibrator_drvdata.dev, "tlmm_motor_suspend");
		...
		if (vibrator_drvdata.flag_en_gpio)
	...
	if (!g_bampenabled) {
		...
		if (vibrator_drvdata.vib_model == HAPTIC_PWM) {
			if(vibrator_drvdata.is_pmic_vib_pwm){ //PMIC PWM
				...
				motor_pinctrl = devm_pinctrl_get_select(vibrator_drvdata.dev, "tlmm_motor_active");
				if (IS_ERR(motor_pinctrl)) {
					if (PTR_ERR(motor_pinctrl) == -EPROBE_DEFER)
						return -EPROBE_DEFER;
